...
export default function OnboardingPage() {
  ...
  const { login, register, isLoggedIn, user, setUser } = useAuth()
  ...
  // Redirect already-authenticated users (login path — role already set)
  if (isLoggedIn && user?.role && screen !== 'role') {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.message || 'Try demo@cima.film / password  or  viewer@cima.film / password')
    }
  }
  ...
  const handleRoleSelect = async (role: 'filmmaker' | 'viewer') => {
    setRoleLoading(true)
    try {
      const res = await usersApi.update({ role })
      setUser(res.data)
    } catch { /* best effort — role defaults to viewer if this fails */ }
            ...
            <div className="flex flex-col gap-4">
              {[
                ...
                <motion.button
                  ...
                  className="w-full bg-card border border-border rounded-2xl p-5 text-left flex items-start gap-4 hover:border-primary hover:shadow-glow-orange/20 transition-all duration-200 interactive-lift disabled:opacity-60"
                >
                  <div className="w-12 h-12 rounded-xl bg-muted flex items-center justify-center shrink-0">
                    {roleLoading ? <Loader2 size={22} className="text-primary animate-spin" /> : <Icon size={22} className="text-primary" />}
                  </div>
                  <div>
